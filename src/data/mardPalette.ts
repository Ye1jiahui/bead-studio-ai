export type MardColor = { code: string; hex: string; rgb: [number, number, number] }

// MARD 标准 221 色（A–H、M 系列）。HEX 为屏幕显示近似值，实体豆可能因批次和光线略有差异。
const RAW = `
...
export const MARD_PALETTE: MardColor[] = RAW.replace(/\s/g, '').split(',').filter(Boolean).map((entry) => {
  const [code, value] = entry.split(':')
  return { code, hex: `#${value}`, rgb: hexToRgb(value) }
})
